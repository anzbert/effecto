...
private let log = Logger(subsystem: "dev.anzio.effectoExtension", category: "AudioUnitViewController")

@MainActor
public class AudioUnitViewController: AUViewController, AUAudioUnitFactory {
    var audioUnit: AUAudioUnit?
    
    var hostingController: HostingController<effectoExtensionMainView>?
	...
	*/
	...
	nonisolated public func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
		return try DispatchQueue.main.sync {
			...
			audioUnit = try effectoExtensionAudioUnit(componentDescription: componentDescription, options: [])
			
			guard let audioUnit = self.audioUnit as? effectoExtensionAudioUnit else {
				log.error("Unable to create effectoExtensionAudioUnit")
				return audioUnit!
			}
			...
			audioUnit.setupParameterTree(effectoExtensionParameterSpecs.createAUParameterTree())
    ...
    private func configureSwiftUIView(audioUnit: AUAudioUnit) {
        ...
        let content = effectoExtensionMainView(parameterTree: observableParameterTree)
