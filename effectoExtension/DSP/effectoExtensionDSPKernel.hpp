...
public:
    ...
    void setParameter(AUParameterAddress address, AUValue value) {
        switch (address) {
            case effectoExtensionParameterAddress::gain:
                mGain = value;
                break;
                // Add a case for each parameter in effectoExtensionParameterAddresses.h
        }
    }
    ...
    void process(std::span<float const*> inputBuffers, std::span<float *> outputBuffers, AUEventSampleTime bufferStartTime, AUAudioFrameCount frameCount) {
        /*
         ...
         modify the check in [effectoExtensionAudioUnit allocateRenderResourcesAndReturnError]
         */
